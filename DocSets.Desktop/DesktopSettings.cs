...
internal sealed class DesktopSettings
{
    ...
    public string LastDocSet { get; set; } = "";
    public string SelectedItemId { get; set; } = "";
    public HashSet<string> ExpandedItemIds { get; set; } = new(StringComparer.OrdinalIgnoreCase);
...
internal sealed class DesktopSettingsStore
{
    private readonly string directory = Path.Combine(
        Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), "DocSets", "Desktop");

    public string SettingsPath => Path.Combine(directory, "settings.json");
    public string LayoutPath => Path.Combine(directory, "layout.xml");
    ...
    public void DeleteLayout()
    {
        ...
        catch (Exception exception) { DocSetsLog.Current.Error("Layout", "Не удалось удалить layout.", exception); }
    }
}
